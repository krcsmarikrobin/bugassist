...
// a C érték optimalizálásához
	class ComputeCValue implements Runnable {
		...
		public void run() {
			...
			filesToCGridSearch.add(
					new File(path + "/OuterFiles/" + "model_from_folds" + foldsCount + "_" + cValueCount + ".dat"));
			...
			try {
				...
				while ((line = reader.readLine()) != null) {
					if (line.startsWith("Zero/one-error on test set: ")) {
						...
						/* törölni */ System.out.println(
								"C érték: " + c + " " + Double.parseDouble(value) + "% minél kisebb annál jobb");
...
// a C érték optimalizálása
	...
	// kiszámolja az osztályozó alkalmazásával minden fold rangsorát a következõ
	// fold (azaz idõben az eggyel régebbi csomag) train adatként való
	// felhazsnálásával
	public void computeClassify() {
		...
		for (int i = 2; i <= foldsCount; ++i) {
			...
			processBuilder.directory(new File(path + "/OuterFiles/"));
			processBuilder.command(path + "/OuterFiles/svm_rank_learn.exe", "-c", Double.toString(cValue),
					new String("folds" + i + ".txt"), new String("folds" + (i - 1) + "_model_from_folds" + i + ".dat"));
			try {
		...
		for (int i = 1; i < foldsCount; ++i) {
			...
			processBuilder = new ProcessBuilder();
			processBuilder.directory(new File(path + "/OuterFiles/"));
			processBuilder.command(path + "/OuterFiles/svm_rank_classify.exe", new String("folds" + i + ".txt"),
			...
			try {
				...
				File predictionsFileRename = new File(path + "/OuterFiles/folds" + i + "_svm_predictions");
				if (predictionsFileRename.exists())
					predictionsFileRename.delete();

				predictionsFile.renameTo(new File(path + "/OuterFiles/folds" + i + "_svm_predictions"));
	...
	public List<BugAndFilesRel> collectResult() {
		...
		for (int i = (foldsCount - 1); i > 0; --i) {
			...
			try {
				...
				BufferedReader readerPredict = new BufferedReader(
						new InputStreamReader(new FileInputStream(predictFile)));
				...
				// például: 1 qid:8181 1:0.99990654 2:0.99993724 3:0.0 4:1.0 5:2.0
				// #1#1367295#NetworkUtils.java
	...
	// Megadja a pontosságot az elsõ k elemre foldsonként. Visszatér egy int
	// tömbbel, amiben a százalék értékek vannak folsonként sorban.
